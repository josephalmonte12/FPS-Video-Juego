...
public class CarController : MonoBehaviour {
	...
	void Update () 
	{
		...
		vSpeed = GetComponent<Rigidbody>().velocity.magnitude;

		_Speed = GetComponent<Rigidbody>().velocity.magnitude * 3.0f;

		if (controlsEnabled){

			soundcontroller.playerIn = true;
			if(fuel > 0.0){
				fuel -= drivetrain.rpm/100000.0f * Time.deltaTime;
			}
			if(drivetrain.rpm == 800.0f && vSpeed < 0.2 && !Input.GetKey(KeyCode.S)){
				GetComponent<Rigidbody>().drag = 50.0f;
			}else{
		...
		if (Input.GetKey (KeyCode.LeftShift) && controlsEnabled && fuel > 0.0)
		{
			throttle += Time.deltaTime / throttleTime;
			throttleInput += Time.deltaTime / throttleTime;
		}
		else if (accelKey && controlsEnabled && fuel > 0.0)
		{
	...
	/*void OnGUI (){
		...
		//var speed = GetComponent<Rigidbody>().velocity.magnitude * 3.0f;
		if(controlsEnabled){
			GUI.Label (new Rect(120, Screen.height - 50, 150, 80), "" + hitPoints, style1);
			GUI.Label (new Rect(20, Screen.height - 50, 150, 80)," HP: ");
			
			GUI.Label (new Rect(Screen.width - 90,Screen.height-50,200,80), "" + fuel.ToString("f2"), style2);
			GUI.Label (new Rect(Screen.width - 160,Screen.height-50,200,80)," Fuel: ");
			
			GUI.Label (new Rect(Screen.width - 80,Screen.height-113,200,80), "" + speed.ToString("F0"), style3);
			GUI.Label (new Rect(Screen.width - 160,Screen.height-113,200,80)," KM/H : ");
		
			//GUI.Label (new Rect(100, Screen.height - 90,100,400), "" + fuel.ToString("F2"), style1);
			//GUI.Label (new Rect(40, Screen.height - 90,100,400),"Fuel : ");
