...
public class OrbController : MonoBehaviour 
{
	...
	private Vector3 aimTarget { get { return player.transform.position + new Vector3(0, 0, 0); } }
	
	// Use this for initialization
	void Start () {
		player = (GameObject) GameObject.Find("TargetPoint");
		//player = (GameObject) GameObject.Find("TargetPoint");
	...
	void FixedUpdate () {
		...
		if (unfolded && GetComponent<Animation>()["Take 001"].enabled == false) {
	...
	public void FoldoutTurret () {
		...
		if (TargetIsInFront() && CanShootTarget()) {
			player.SendMessage("OnPanic", SendMessageOptions.DontRequireReceiver);
			gotBehindYet = false;
			hitsThisRound = 0;
	...
	public void Shoot () {
		...
		RaycastHit hit;
		if (Physics.Raycast(ray, out hit, 1000, mask)) {
			hit.transform.root.SendMessage("ApplyDamage", 20, SendMessageOptions.DontRequireReceiver);
			//hit.gameObject.SendMessage("ShowHitDirection", 2, SendMessageOptions.DontRequireReceiver);
	...
	void OnHit (RayAndHit rayAndHit) {
		...
		if (Random.value < 0.2f)
			GetComponent<Rigidbody>().AddForceAtPosition(rayAndHit.ray.direction * 15, rayAndHit.hit.point, ForceMode.Impulse);
