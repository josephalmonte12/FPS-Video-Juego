		...
		if (editLinks) {
			...
			if (Event.current.shift) {
				...
				if (firstShiftNode != null) {
					...
					Handles.SphereHandleCap (GUIUtility.GetControlID (FocusType.Passive),(Vector3)firstShiftNode.position,Quaternion.identity,HandleUtility.GetHandleSize ((Vector3)node.position)*0.12F, EventType.Repaint);
				}
				
				if (node != null) {
					Handles.color = Color.yellow;
					Handles.SphereHandleCap(GUIUtility.GetControlID (FocusType.Passive),(Vector3)node.position,Quaternion.identity,HandleUtility.GetHandleSize ((Vector3)node.position)*0.13F, EventType.Repaint);
		...
		for (int i=0;i<conns.Length;i++) {
			...
			if (conn.type == ConnectionType.Connection) {
				if (conn.oneWay) {
					...
					Handles.ConeHandleCap (controlID,conn.p1+goldenRatio,Quaternion.LookRotation (goldenRatio),goldenRatio.magnitude*0.07F, EventType.Repaint);
					Handles.ConeHandleCap(controlID,conn.p2-goldenRatio,Quaternion.LookRotation (goldenRatio),goldenRatio.magnitude*0.07F, EventType.Repaint);
				...
				Handles.SphereHandleCap (controlID,conn.p1,Quaternion.identity,HandleUtility.GetHandleSize (conn.p1)*0.1F, EventType.Repaint);
				Handles.SphereHandleCap (controlID,conn.p2,Quaternion.identity,HandleUtility.GetHandleSize (conn.p2)*0.1F, EventType.Repaint);
			} else {
				if (conn.enable) {
					//Nice Blue Color
					Handles.color = new Color (0.290F, 0.454F, 0.741F, 0.600F);
				...
				Handles.SphereHandleCap (controlID,conn.p1,Quaternion.identity,HandleUtility.GetHandleSize (conn.p1)*0.15F, EventType.Repaint);
			}
		}
